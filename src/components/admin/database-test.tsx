...
export function DatabaseTest() {
    ...
    try {
      ...
      if (profile?.role === 'admin') {
        ...
        if (seriesError) {
          ...
          addResult(`✅ Série de test trouvée: ${testSeries[0].name}`);
          ...
          if (questionError) {
            ...
            addResult(`   Hint: ${questionError.hint || 'Aucun hint'}`);
          } else {
            ...
            if (optionsError) {
              ...
              addResult(`   Code: ${optionsError.code}`);
              addResult(`   Détails: ${optionsError.details || 'Aucun détail'}`);
            } else {
            ...
            if (deleteError) {
              addResult(`❌ Suppression question: ${deleteError.message}`);
            } else {
              addResult(`🧹 Question de test supprimée`);
            }
          }
        } else {
          addResult(`❌ Aucune série de test trouvée`);
        }
      } else {
        addResult("4. ⚠️ Utilisateur non admin - permissions limitées");
      }
  ...
  return (
    <div className="space-y-6">
      <Card>
        <CardHeader>
          <CardTitle>Test des permissions de base de données</CardTitle>
        </CardHeader>
        <CardContent>
